---
name: knowlink-frontend-standards
description: Project-specific conventions for the KnowLink frontend (React 19 + TypeScript + Vite). Consult this skill any time you write, edit, review, or discuss code for the knowlink-frontend repo — creating or modifying components, hooks, API functions, Zod schemas, forms, routes, or types; deciding where a new file belongs in the module structure; naming files, functions, hooks, or types; handling API errors; working with backend enums; or answering questions about folder structure, state management, or formatting rules. Apply these conventions silently and by default, even if the user doesn't explicitly ask for "the standards" — this is not optional style guidance, it's how this codebase is written.
---
...
Este skill cubre la **aplicación web** del sistema.
